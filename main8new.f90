...
implicit none 
    ...
    real(kind=8):: r1plus(10000,4), r1minus(10000, 4) !массивы вихрей и антивихрей
    integer:: Nplus=1, Nminus=1
    real(kind=8) :: minus_1=-1., plus_1=1., flag1
    real(kind=8) :: defs(4000,4000), stepx,stepy,Up

    real(kind=8) :: released_E_num, j_
    flag1=1.
    ...
    do j = 1, Nminus-1     !Считаем энергию антивихрей
        ...
        call Usurface1(U0, K0tab, Usurf, lambda, tabul, r1minus, r1plus,&
            Nplus, Nminus, width, length, minus_1, j)
        Uj = Uj + 1./2.*Usurf
        call Ucurrent(r1minus(j,3), r1minus(j,1), H0, lambda, length, HI, Um_Ut, minus_1)
        ...
        ! call defects(defs, r1minus(j, 1:2), length, width, Up, stepx, stepy)
        !         Uj = Uj + Up
    end do
    ...
    do j = 1, Nplus-1     !Считаем энергию вихрей
        ...
        Uj = Uj + 1./2.*Usurf
        !write(*,*) Uj
        call Ucurrent(r1plus(j,3), r1plus(j,1), H0, lambda, length, HI, Um_Ut, plus_1)
        Uj = Uj + Um_Ut*delta*10.**(-7.)
        !write(*,*) Uj
        do k = 1, Nplus-1    
            if( j .NE. k) then
        ...
        ! call defects(defs, r1plus(j, 1:2), length, width, Up, stepx, stepy)
        !         Uj = Uj + Up
    end do
    ...
    ! write(45,*) H0, H0 - float(Nplus-Nminus) * phi0 / (length * width * 10.0 ** (-14.0))
    ! write(*,*) H0, H0 - float(Nplus-Nminus) * phi0 / (length * width * 10.0 ** (-14.0));
    I_ = HI*2*0.0001
    j_ = I_/(length*10**(-7.)*width*10**(-7.))
    write(45, *) j_*0.000001 , released_E_num/100.!мегаамперы
    write(*,*) HI
    if(HI<1000. .and.  flag1>0.) then
        HI = HI+50.
    else
        flag1= -1.
        HI=HI-100.
    end if
    if(abs(HI+1000.)<50.) then
        flag1=1.
    end if
    end do
...
44  write(*,*)'in the end', Uj
    ...
    do j = 1, Nplus-1     !Считаем энергию вихрей
        ...
        ! call defects(defs, r1plus(j, :2), length, width, Up, stepx, stepy)
        !         Uj_end = Uj_end + Up
        ! write(*,*) 'Uij+-', Uj_end
    end do
    write(*,*) Uj_end +float(Nplus+Nminus-2) * (epss), Nplus-1, Nminus-1
    !write(*,*) r1(1:11,:)
    do i = 1, Nplus-1
        write(2, *) r1plus(i, 1:2)
    end do
    write(2, *) 'flag'
    do i = 1, Nminus-1
        write(2, *) r1minus(i, 1:2)
    end do
    ...
    contains
    !Расстояние для одной периодичной и одной непериодичной границ
    ...
    subroutine Ucurrent(side, x, H0, lambda, length, HI, Um_Ut, signum_)
        ...
        + HI*signum_*(sinh(x/lambda)/sinh(length/(2.*lambda))-side1))
        Um_Ut = Um_Ut * 6.2 * 10.**(11.)
        
    end subroutine

    !**************************
    !Взаимодействие с дефектами вихрей
    !**************************
    subroutine defects(defs, vort, length, width, Up, stepx, stepy)
        real(kind=8) :: defs(4000,4000), T, vort(2), xi, Up, alpha, U0
    ...
    subroutine creation(lambda, length, width, H0, HI, epss, Nminus, r1minus, &
        Nplus, r1plus, tabul, delta, tauNa, T, Uj, U0, K0tab, xi,&
        defs, stepx, stepy)
        real(kind=8)::r1minus(10000,4), r1plus(10000,4)
        ...
        if(coordcreate(3)>0.) then
            r1plus(Nplus,1:2) = coordcreate(1:2)
            r1plus(Nplus,3) = side
            call Usurface(U0, K0tab, Usurf, lambda, tabul, r1minus, r1plus,&
        ...
        Nplus, Nminus, width, length, coordcreate(3), Nplus)
            ...
            r1minus(Nminus,1:2) = coordcreate(1:2)
            r1minus(Nminus, 3) = side
            call Usurface(U0, K0tab, Usurf, lambda, tabul, r1minus, r1plus,&
        ...
        dE = dE + Usurf
        call defects(defs, coordcreate(1:2), length, width, Up, stepx, stepy)
        dE = dE + Up
        ...
        r1plus(Nplus,1:3)=100000.
        r1minus(Nminus, 1:3) =100000.
        r1plus(Nplus,4)=0.
        r1minus(Nminus, 4) =0.
    end subroutine
    ...
    subroutine annihilation(Uj, lambda, tauNa, T, K0tab, width, length, epss,&
     H0, HI, U0, delta, tabul, r1plus, r1minus, Nplus, Nminus,&
     defs, stepx, stepy, released_E_num)
        real(kind = 8):: signum_anihil, random_anihil, coordanihil(4)
        ...
        real(kind=8):: r1plus(10000, 4), r1minus(10000, 4)

        real(kind=8)::defs(4000,4000),stepx,stepy,Up

        real(kind=8) :: released_E_num
        ...
        if(signum_anihil>0.5) then
            ...
            coordanihil(1:2) = r1plus(number, 1:2)
            coordanihil(3) = 1. !Знак вихря
            coordanihil(4) = r1plus(number, 3) !С какой стороны родился
            flag = 1.
            end if
        else
            call random_number(random_anihil)
            ...
            coordanihil(1:2) = r1minus(number, 1:2)
            coordanihil(3) = -1. !Знак вихря
            coordanihil(4) = r1minus(number, 3)
            flag = 1.
            end if
        end if
        if (flag>0.) then
        if (length/ 2. - abs(coordanihil(1)) < lambda .and. length/ 2. - abs(coordanihil(1))>0.4*lambda) then 
            ...
            call defects(defs, coordanihil(1:2), length, width, Up, stepx, stepy)
            dE = dE + Up
            ...
            if (Wa > random_anihil) then
                if(coordanihil(3)>0) then
                    if(r1plus(number, 3)*r1plus(number, 1)<0.) then
                        released_E_num = released_E_num + 1.
                    end if
    ...
    subroutine movement(Uj, lambda, T, length, width, epss, tabul, H0,&
        ...
        real(kind=8) :: coordnew(4), signum_move, temp, coordold(2)
        integer :: number_move, j, i_move 

        real(kind=8) :: randd, poschange, angle, xchange, ychange, num_temp, flag
        real(kind=8)::defs(4000,4000),stepx,stepy,Up

        real(kind=8):: Ucurrent_diff, phi0 =  2.06783460999999E-07
        dE = 0.0
        pi = acos(-1.0)
        ...
        Ucurrent_diff = 0.000000000000000
            call Ucurrent(coordnew(4), coordnew(1), H0, lambda, length, HI, Um_Ut, coordnew(3))
            dE = dE + Um_Ut*delta*10.**(-7.)
            if(coordnew(3)>0.) then
                call Ucurrent(r1plus(number_move, 3), r1plus(number_move, 1), H0, lambda, length, HI, Um_Ut, coordnew(3))
                dE = dE - Um_Ut*delta*10.**(-7.)
                ! call defects(defs, r1plus(number_move, :), length, width, Up, stepx, stepy)
                ! dE = dE - Up
            else
                call Ucurrent(r1minus(number_move, 3), r1minus(number_move, 1), H0, lambda, length, HI, Um_Ut, coordnew(3))
                dE = dE - Um_Ut*delta*10.**(-7.)
                ! call defects(defs, r1minus(number_move, :), length, width, Up, stepx, stepy)
                ! dE = dE - Up
            end if

            ! call defects(defs, coordnew(1:2), length, width, Up, stepx, stepy)
            ! dE = dE + Up
            ...
            if(coordnew(3)>0.) then
                coordold = r1plus(number_move,1:2)
                r1plus(number_move,1:2) =coordnew(1:2)
            else
                coordold = r1minus(number_move,1:2)
                r1minus(number_move,1:2) =coordnew(1:2)
            end if
            call Usurface(U0, K0tab, Usurf, lambda, tabul, r1minus, r1plus,&
                Nplus, Nminus, width, length, coordnew(3), number_move)
            dE = dE + Usurf
            if (dE<0) then
                    Uj = Uj + dE
                    ! if(coordnew(1)*coordold(1)<0.) then
                    !     ! write(*,*) Ucurrent_diff
                    !     Ucurrent_diff = 2.*HI*phi0/(4.*pi)*delta*10.**(-7.)*6.2*10.**(11.)
                    !     Ucurrent_diff = Ucurrent_diff  * coordnew(3) * coordnew(4)
                    !     Uj = Uj + Ucurrent_diff  
                    ! end if
            else
                possib1 = 1.0 / e ** ( dE / T)
                ...
                if(possib1>randd) then
                    ...
                    if(coordnew(3)>0.) then
                        r1plus(number_move,1:2) = coordold
                    else
                        r1minus(number_move,1:2) = coordold
                    end if
                end if
            end if
        !if((i > iter - 11)) then
    ...
    subroutine Usurface(U0, K0tab, Usurf, lambda, tabul, r1minus, r1plus,&
     Nplus, Nminus, width, length, signum_, number)
        ...
        real(kind=8):: U0, K0tab(10000), r1plus(10000,4), r1minus(10000,4), ri(3),lambda,tabul
        real(kind=8):: xvort, xdiff, ydiff, ximage, width, length, rij, Usurf
        integer :: Nplus, Nminus, i5, i6, i7, j, number
        real(kind=8):: signum_,flag10
        Usurf =0.
        if(signum_>0.) then
            ri(1:2) = r1plus(number, 1:2)
            ri(3) = 1.
        else if (signum_<0.) then
            ri(1:2) = r1minus(number, 1:2)
            ri(3) = -1.
        end if
        ...
        if(signum_>0.) then
            do j =1, Nminus-1
                !r1(j,3)- знак вихря, далее к координате прибавляем 2 расстояния если справа, отнимаем два расст если слева
    ...
    epss,r1plus,r1minus,Nplus,Nminus, H0,HI, U0, K0tab,&
    defs, stepx,stepy, released_E_num)
        real(kind=8):: lambda, xi, epss,r1plus(10000,4),r1minus(10000,4), plusminus
        real(kind=8):: width, length, rplus(4), rminus(4), xdiff, ydiff, rij
        ...
        real(kind=8)::defs(4000,4000),stepx,stepy,Up, gfg
        ...
        Nplus, Nminus, width, length, ri(3), Nminus_)
            dE = dE + Usurf
            !write(*,*) 'Usurf2=', Usurf
            call defects(defs, r1minus(Nminus_,1:2), length, width, Up, stepx, stepy)
            dE = dE + Up
            call defects(defs, r1plus(Nplus_,1:2), length, width, Up, stepx, stepy)
            dE = dE + Up

            !Два раза учли их взаимодействие с изображениями друг друга
            ...
            length * 10**(-7.)*width*10**(-7.)) * exp((1.0 / T) * (dE))
                ...
                if (Wa > pairanihil) then
                    if(r1plus(Nplus_,1)*r1plus(Nplus_, 3)<0.) then
                        Ucurrent_diff = 2.*HI*phi0/(4.*pi)*delta*10.**(-7.)*6.2*10.**(11.)
                        Ucurrent_diff = Ucurrent_diff  * r1plus(Nplus_, 3)
                        Uj = Uj - Ucurrent_diff  
                    end if
                    if(r1minus(Nminus_,1)*r1minus(Nminus_, 3)<0.) then
                        Ucurrent_diff = 2.*HI*phi0/(4.*pi)*delta*10.**(-7.)*6.2*10.**(11.)
                        Ucurrent_diff = Ucurrent_diff  * r1minus(Nminus_, 3)
                        Uj = Uj + Ucurrent_diff  
                    end if
                    r1plus(Nplus_, :) = r1plus(Nplus-1, :)
                    r1plus(Nplus-1, 1:3) = 100000.
                    r1plus(Nplus-1, 4) = 0.
                    Nplus = Nplus-1

                    r1minus(Nminus_, :) = r1minus(Nminus-1, :)
                    r1minus(Nminus-1, 1:3) = 100000.
                    r1minus(Nminus-1, 4) = 0.
                    Nminus = Nminus-1

                    Uj = Uj - dE
                    
                    ! write(*,*) dE, Nplus, Nminus
                    released_E_num = released_E_num+1.
                    ! write(*,*) '9'
                end if
        end if
