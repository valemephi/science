...
implicit none 
    ...
    integer :: N = 1, i, j, k, i4, i1, iter, a, b, nomera(-200:200), i2, i5
    ...
    epss = (phi0 / (4 * pi * lambda * 10 ** (-7.0))) ** 2 * (log(lambda / xi) + 0.45)  * delta * 10 ** (-7.0) *6.2 * 10.0**(11)
    !write(*,*) epss
    ...
    do i = 1, floor(tabul)
        ...
        temp = (phi0**2.0 / (16.0 * pi**2.0 * (lambda**2.0 * 10 ** (-14.0)))) * delta * 10**(-7.0)
        !temp = (phi0**2.0 / (16.0 * pi**2.0 * (lambda * 10 ** (-7.0))**2.0)) * delta * 10**(-7.0)
        !Utab(i) = temp * bk0
        Utab(i) = temp * (-(-lambda*2.3/x**2 + 2/x))*lambda*10**(-7.0)*0.1
    end do
    ...
    H = H + 50.0
    Uh = (-1.0) * phi0 * H * delta * 10**(-7.0)/ (4.0 * pi) * 6.2 * 10 ** (11.0)
    write(3,*) H, H - N * phi0 / (length * width * 10.0 ** (-14.0))
