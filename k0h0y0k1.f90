...
implicit none 
    ...
    open(77, file='tabmaspy_info10.txt')
    read(77, *) Utabpy
    close(77)
    Utab= Utabpy
    ...
    do i = 1, iter
        ...
        do i1 = 1, N1
            ...
            if(process < 0.4) then
                call movement(Uj, r1, N, lambda, T, Utab, length, width, epss, Uh, tabul, H0, HI, U0, K0tab, delta)
            else if(process < 0.7) then
                call creation(Uj, r1, N, lambda, tauNa, T, width, length, Utab, epss,Uh, H0, HI, U0, K0tab, delta, tabul)
                
            else if (N .NE. 1) then
                call annihilation(Uj, r1, N, lambda, tauNa, T, Utab, width, length, epss, Uh, H0, HI, U0, K0tab, delta, tabul)
            end if
        end do 
...
44    Ujj = 0.0
    ...
    !РОЖДЕНИЕ
    !****************************
    subroutine creation(Uj, r1, N, lambda, tauNa, T, width, length, Utab, epss, Uh, H0, HI, U0, K0tab, delta, tabul)
    ...
    subroutine annihilation(Uj, r1, N, lambda, tauNa, T, Utab, width, length, epss, Uh, H0, HI, U0, K0tab, delta, tabul)
        ...
        !write(*,*) number
        if (length/ 2. - abs(r1(number, 1)) < lambda) then 
    ...
    !Движение
    !***************
    subroutine movement(Uj, r1, N, lambda, T, Utab, length, width, epss, Uh, tabul, H0, HI, U0, K0tab, delta)
