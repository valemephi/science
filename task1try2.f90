...
program task1
    ...
    real(kind = 8) :: T, Tc
    integer :: N = 100, i, j, j1, k, i1, i2, i3, sec, sec1, sec10, sec20
    ...
    real(kind = 8) :: x, x1, x2, y1, y2, ydiff, xdiff , x3, x4, y3, y4, ydiff2, xdiff2, rij2, x10, temp10 !Координаты вихрей
    real(kind = 8) :: r(100,2)
    real(kind = 8) :: pi
    real(kind = 8) :: epss  ! Собственная энергия всех вихрей
    real(kind = 8):: Ue(100), r1(100,2), r2(100,2), sect(40, 40)
        ...
        do i1 = 1, N
            ...
            do j = 1, 100         !Считаем     !начальную энергию этой итерации
                if( j.NE. number ) then
                    ...
                    temp = (phi0**2 / (8.0 * pi**2 * lambda**2)) * delta * bk0
                    Unewone = temp + Unewone
                end if
            end do
            do j1 = 1, 100         !Считаем     !начальную энергию этой итерации
                if( j1 .NE. number ) then
                    ...
                    y4 = r1(number, 2)
                    if(y3>y4) then
                        if(5-y3+y4> y3-y4) then
                            ydiff2 = y3-y4
                        else
                            ydiff2 = 5-y3+y4
                        end if
                    else
                        if(5-y4+y3> y4-y3) then
                            ydiff2 = y4-y3
                        else
                            ydiff2 = 5-y4+y3
                        end if
                    ...
                    if(x3>x4) then
                        if(5-x3+x4> x3-x4) then
                            xdiff2 = x3-x4
                        else
                            xdiff2 = 5-x3+x4
                        end if
                    else
                        if(5-x4+x3> x4-x3) then
                            xdiff2 = x4-x3
                        else
                            xdiff2 = 5-x4+x3
                        end if
                    end if   
                    rij2 = sqrt(ydiff2**2 + xdiff2**2)
                    x10 = (rij2*10.0**(-4) / lambda)
                
                    call besssel(x10,bi0, di0, bi1, di1, bk0, dk0, bk1, dk1)
                
                    temp10 = (phi0**2 / (8.0 * pi**2 * lambda**2)) * delta * bk0
                    Uoldone = Uoldone + temp10
                end if
            end do
    ...
    do i = 1, iter
        ...
        !вывод жнергии каждые 10 итераций
            if (mod(i,10)==0) then
                write(*,*) i, 'Uj=', Uj
            !write(*,*) 'dE=', dE
            !write(*,*) 'ychange=', ychange
            !write(*,*) 'xchange=', xchange
            end if
...
subroutine annihilation(Uj, r1, N, lambda, tauNa, T, Utab, width, length, epss, Uh)
    ...
    real(kind=8) :: pa, Wa, dE, Uj, r1(N, 2), lambda, tauNa, Utab(10000), width, length, T, epss, Uh
