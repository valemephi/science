...
implicit none 
    ...
    integer :: number, N1, maxvort=10000, i5
    real(kind = 8) :: process
    tauNa = 100.0
    T = 1.0
    xi0 = 2.0
    ...
    tabul = 10000.0
    length = 5.0*1000.0 ! по х
    width = 5.0*1000.0   !по у
    Tc = 84.0
    H = 160.0
    ...
    do i = 1, floor(tabul)
        ...
        !Utab(i) = temp * bk0
        temp = (phi0**2.0 / (16.0 * pi**2.0 * (lambda * 10 ** (-14.0))))*delta*10**(-7.0)
        !temp = (phi0**2.0 / (16.0 * pi**2.0 * (lambda * 10 ** (-7.0))**2.0)) * delta * 10**(-7.0)
        !Utab(i) = temp * bk0
        Utab(i) = temp * (-(-lambda*2.3/x**2 + 2/x))*lambda*10**(-7.0)
    end do

    write(*,*) Utab(:10), Utab(9990:)
    Utab = Utab * 6.2 * 10.0 **(11.0)
    Uj = 0.0
    !countmed = 1
! do i5 = 1, 20
! write(*,*) 'Iteration number ', i5
! H = H + 50.0
Uh = (-1.0) * phi0 * H * delta * 10**(-7.0)/ (4.0 * pi) * 6.2 * 10 ** (11.0)
    do i = 1, iter
        if(N>maxvort) then
