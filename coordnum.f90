...
implicit none 
    ...
    integer :: number, N1
    real(kind = 8) :: process
    real(kind=8):: dist1,S6, angle6, s61
    integer :: num6
    tauNa = 100.0
    T = 1.0
    xi0 = 2.0
...
44    Ujj = 0.0
    ...
    do i = 1, N-1
        num6 =0
        do k = 1, N-1
            call dist(r1,i,k, width, length, dist1)
            if(i .NE. k .and. dist1<2*lambda) then
            num6 = num6+1
            angle6 = atan((r1(i,2)-r1(k,2))/(r1(i, 1)-r1(k,2)))
            S61= S61 + exp(6*angle6)
            end if
        end do
        S6 = S6+S61/num6
    end do
    ...
    !РОЖДЕНИЕ
    !****************************
    subroutine dist(r1, i, k, dist1,width,length)
    real(kind=8)::r1(10000,2),x1,dist1,x2,y1,y2,ydiff,xdiff, length, width
    integer::i,k
    x1 = r1(i, 1)
    x2 = r1(k, 1)
    y1 = r1(i, 2)
