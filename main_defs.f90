...
implicit none 
    ...
    r1plus(10000,2)=100000.
    r1minus(10000,2)=100000.
    ! endd = 100
    ...
    do j = 1, Nminus-1     !Считаем энергию антивихрей
        ...
        call defects(defs, r1minus(j, :), length, width, Up, stepx, stepy)
                Uj = Uj + Up
    end do
    ...
    do j = 1, Nplus-1     !Считаем энергию вихрей
        ...
        call defects(defs, r1plus(j, :), length, width, Up, stepx, stepy)
                Uj = Uj + Up
    end do
...
44  write(*,*)'in the end', Uj
    ...
    subroutine movement(Uj, lambda, T, length, width, epss, tabul, H0,&
        ...
        if (abs(coordnew(1) + xchange) < length/2.) then
            ...
            if(coordnew(3)>0.) then
                ...
                r1plus(number_move,:) =coordnew(1:2)
            else
                coordold = r1minus(number_move,:)
                r1minus(number_move,:) =coordnew(1:2)
            end if
            call Usurface(U0, K0tab, Usurf, lambda, tabul, r1minus, r1plus,&
                Nplus, Nminus, width, length, coordnew(3), number_move)
            dE = dE + Usurf
            ...
            if (dE<0) then
                ...
                if(possib1>randd) then
                    ...
                    if(coordnew(3)>0.) then
                        r1plus(number_move,:) = coordold
                    else
                        r1minus(number_move,:) = coordold
                    end if
                end if
            end if
        !if((i > iter - 11)) then
    ...
    defs, stepx,stepy)
        ...
        Nplus, Nminus, width, length, ri(3), Nminus_)
            ...
            call defects(defs, r1minus(Nminus_,:), length, width, Up, stepx, stepy)
            dE = dE + Up
            call defects(defs, r1plus(Nplus_,:), length, width, Up, stepx, stepy)
            dE = dE + Up
